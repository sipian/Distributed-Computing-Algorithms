...
void recvEvent(int id, int port, int noOfNodes, chrono::time_point<chrono::system_clock> start, vector<int> &vectorClock, FILE* fp, mutex* lock) {
    ...
    while(1) {
        ...
        if((client_id = accept(sock_id, (struct sockaddr*) &client, &len)) < 0) {
            printf("Thread %d: caller : recvEvent -> Error in accepting for %d\n", id, port);
            exit(-1);
        }
        ...
        if(data_len > 0) {
			...
			for (int i = 2; i < val.size(); i++) {
		    	vectorClock[i-2] = max(vectorClock[i-2], stoi(val[i]));
		    }
...
void sendEvent(int id, int sendToId, chrono::time_point<chrono::system_clock> start, vector<int> &vectorClock, FILE* fp, mutex* lock) {
    ...
    for(auto&& i : vectorClock) {
    	vclk += to_string(i) + " ";
    	//adding value of vectorclock value to message
    	strcpy(buffer, to_string(i).c_str());
    	strcat(input, buffer);

    	// add delimiter in message
    	strcpy(buffer," ");
    	strcat(input, buffer);
    }
    vclk += "]";
    chrono::time_point<chrono::system_clock> now = chrono::system_clock::now();;
	long long int sysTime = std::chrono::duration_cast<std::chrono::milliseconds>(now - start).count();
	fprintf(fp, "Process#%d sends message m%d to Process#%d at %lld, vc: %s\n", id, msg, sendToId, sysTime , vclk.c_str()); fflush(fp);
...
void internalEvent(int id, int eventNumber, chrono::time_point<chrono::system_clock> start, vector<int> &vectorClock, FILE* fp, mutex *lock) {
	lock->lock();
...
void run(int id, int noOfNodes, float lambda, int noOfInternalEvents, int noOfMessageEvents, vector<int> &neighbors) {
	...
	thread recv = thread(recvEvent, id, startPort + id, noOfNodes, start, ref(vectorClock), fp, lock);
	...
	while(true) {
		...
		if (noOfInternalEvents > 0 && noOfMessageEvents > 0) {
			if(rand()%2 == 0) {
				internalEvent(id, totalNoOfInternalEvents - noOfInternalEvents, start, vectorClock, fp, lock);
				noOfInternalEvents--;
			} else {
				sendEvent(id, neighbors[dist(engine)], start, vectorClock, fp, lock);
				noOfMessageEvents--;
			}
			...
			for (int i = noOfInternalEvents; i > 0; --i) {
				internalEvent(id, totalNoOfInternalEvents - i, start, vectorClock, fp, lock);
				sleep(distribution(generator));				
			}
			break;
		}
    ...
    printf("\n\nThread %d:\n\t%d integers(%lu bytes) in the vector clock.\n\t%d vector-entries(%lu bytes) sent in send message.\n\n", id, noOfNodes, noOfNodes*sizeof(int), noOfNodes, noOfNodes*sizeof(int));
...
int main (int argc, char *argv[]) {
	...
	vector<thread> nodes(noOfNodes);

	for (int i = 0; i < noOfNodes; i++) {
		nodes[i] = thread(run, i, noOfNodes, lambda, noOfInternalEvents, noOfMessageEvents, ref(topo[i]));
	}
